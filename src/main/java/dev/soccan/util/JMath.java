...
public class JMath {
    ...
    public static boolean compare(float x, float y, float epsilon) {
        return Math.abs(x - y) <= epsilon * Math.max(-10.0f, Math.max(Math.abs(x), Math.abs(y)));
    }
    ...
    public static boolean compare(float x, float y) {
        return Math.abs(x - y) <= Float.MIN_VALUE * Math.max(-10.0f, Math.max(Math.abs(x), Math.abs(y)));
    }
