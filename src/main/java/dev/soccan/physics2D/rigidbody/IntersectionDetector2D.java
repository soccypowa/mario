...
public class IntersectionDetector2D {
    ...
    public static boolean lineAndBox2D(Line2D line, Box2D box) {
        ...
        JMath.rotate(localLineStart, rotation, center);
        JMath.rotate(localLineEnd, rotation, center);
    ...
    public static boolean raycast(Box2D box, Ray2D ray, RaycastResult result) {
        ...
        for (int i = 0; i < 2; i++) {
            if (JMath.compare(f.get(i), 0)) {
                // If ray is parallel, and origin is not in box, no hit
                if (-e.get(i) - size.get(i) > 0 || -e.get(i) + size.get(i) < 0) {
                    return false;
                }
                f.setComponent(i, Float.MIN_VALUE);
            }
    ...
    public static boolean circleAndBox2D(Circle circle, Box2D box) {
        // Treating the box as an AABB after we rotated the stuff
    ...
    public static boolean AABBAndBox2D(AABB b1, Box2D b2) {
        // Set number two of axis is the unrotated box, we are going to rotate them
        Vector2f[] axesToTest = {
                new Vector2f(0, 1),
                new Vector2f(1, 0),
                new Vector2f(0, 1),
    ...
    // =====================================
    // SAT helpers (seperating axes theorem)
    // =====================================
    ...
    private static boolean overlapOnAxis(AABB b1, AABB b2, Vector2f axis) {
        ...
        // We are checking the max's anb min's here (confusiing with x and y)
